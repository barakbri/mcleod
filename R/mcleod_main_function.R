...
#'
#' @details For a prior of type \code{MCLEOD.PRIOR.TYPE.BETA.HEIRARCHICAL}, the Polya tree defines a piecewise constant prior with 2^(Beta.Heirarchical.Levels-1) segments on the real line. For priors defined using \code{MCLEOD.PRIOR.TYPE.TWO.LAYER.DIRICHLET}, the number of segments on the real line is defined using \code{Two.Layer.Dirichlet.Intervals}, and the number of nodes in the upper later of the dirichlet tree is defined by \code{Two.Layer.Dirichlet.Nodes.in.First.Layer}. .The support for the prior is given by the parameter \code{a.limits} in the function \code{\link{mcleod}}, with "jumps" in the piecewise constant density function equally placed equally across the support defined by \code{a.limits}.
#' 
...
#' 
#' @details The default prior assumed for \eqn{\vec{\delta}} is a multivariate normal prior, with mean of 0, and a covariance matrix given by \code{diag(beta_prior_sd)}. Setting one of the standard deviations to a negative value means a non-informative prior is assumed for the slope coefficient.
#'
...
#' @param beta_init Initial values for \eqn{\vec{\delta}} in the MCMC sampler.
...
#' 
#' If covariates are also supplied by the user, the algorithm performs two types of MCMC steps: one for the density of the random intercept term (density of  \eqn{\gamma_i's}) and one for the vector of slope coefficients (given by \eqn{\vec{\delta}}). MCMC steps for the slope coefficients are done using an accept-reject Metropolis-Hastings step. The function \code{\link{results.covariate.coefficients.posterior}} allows the mean posterior estiamtes of \eqn{\vec{\delta}} to be extracted from the output, together with plots and statistics for the posterior distribution of \eqn{\vec{\delta}}, and the acceptance ratio of the MH step.
#' The prior distribution for \eqn{\vec{\delta}}, together with parameters for the proposal distribution (suggesting the MCMC algorithm new values of \eqn{\vec{\delta}} based on the current iteration's values) can be set using the parameter \code{covariates_estimation_parameters}.
#'
