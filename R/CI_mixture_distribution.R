...
#' @param rho.calibration.method one of 'max', 'sum', 'specific': sets how rho is selected via calibration. 'max' chooses the rho with the minimum maximum distance between LE and GE curves for the holdout data; 'sum' chooses 'rho' with the minimum sum of ditances between GE and LE curves; 'specific' chooses different rhos for different values of theta (the natural parameter)
#' @param rho.set.value default is NA, meaning this value of rho (shift parameter when testing LE/GE hypothesis) is set adaptivaly from the data. If this is set to a positive value, this value of rho will be used when testing LE/GE hypotheses. Setting this parameter to a value different than NA will disable the calibration selected by the parameter rho.calibration.method
