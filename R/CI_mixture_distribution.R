...
#' Constructs an object containing the parameters used in the computational pipeline for estimating pointwise confidence intervals for quantiles and percentiles of the mixing distribution, for binomial/poisson generated samples.
#' 
#' @details See package vignette on the meaning of the parameter rho, the procedure for estimating rho, and its parameters.
#'
...
#' @param theta_vec When estimating CIs/ CI curves of the mixing distribution, this vector sets the possible quantiles over which CIs are built (values are deviates of the random effect distribution).
...
#' 
#' The assumed model is of the form \eqn{Xi ~ bin(Ni,Pi)}, where \eqn{i} is an index in the range 1,...,n. The function constructs pointwise confidence intervals for the mixing distribution, i.e., the distribution of Pi's. The statistical method is explained in the package vignette. 
#' 
...
#' @param compute_P_values_over_grid Shoudl 
