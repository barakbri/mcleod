...
The document is structed as follows. In Section 2 we introducing the generative model for the setting with no covariates, and briefly discuss how the estimation procedure for the mixing distribution. In Section 3 we show how point estimates for the CDF of the mixing distribution can be obtained using the package. In Section 4 we breifly discuss how confidence intervals for the quantiles and percentiles of the mixing distribution are constructed and show several code examples. In Section 5 we discuss the different random intercept models implemented in the `mcleod` package and provide example for fitting those models to data. In Section 5 we discuss some advances topics.
...
## Sampling from the posterior distribution
   The MCMC algorithm can be breifly explained as follows:
   (1) Given $\vec{\pi}$, samples for$\gamma_i$ can be drawn from  
