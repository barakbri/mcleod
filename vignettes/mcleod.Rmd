...
```
The simplest way to incorporate covariates in the model is to use the argument named `covariates` to pass a matrix of the covariates. No additional tuning of parameters is needed, but it is best if the covariates are normalized. The model is fitted using the following command:
...
print(coeffs)
```
The MCMC samples for $\vec{\beta}$ can be obtained via `res$additional$original_stat_res$beta_smp`. Note that the values of $\vec{\beta}$ are initiated from $\vec{0}$.

```{r,cache=TRUE,eval=TRUE}
head(t(res$additional$original_stat_res$beta_smp))
```
...
### Example 2: How to set different initialization and change priors for $\vec{\beta}$

In the second example, we have a single covariate and a bimodal random intercept distribution. We show how to (a) change the initial values for $\vec{\beta}$ and (b) how to change the prior. The next code snippet generates the example data:
...
We choose better init values for $\vec{\beta}$, by running a binomial regression assuming the $\gamma_i$'s are normal. The results are used as init for our model.
...
formula_test = paste0("cbind(c,nc)~ ",paste0('Z',1:n_Z,collapse = ' + ')," + (1|obs)");
...
In order to change the init for the MCMC algorithm, we pass `mcleod_init_basic` as an argument to `mcleod.covariates.estimation.parameters`. In addition, we set the SD for the prior for beta to $1.5$.
```{r,cache=TRUE,eval=TRUE}
  
  #Generate object with parameters for covariate estimation
  coeffs_obj  = mcleod.covariates.estimation.parameters(
                  beta_init = mcleod_init_basic,
                  beta_prior_sd = c(1.5))
...
We obtain the posterior slope in the next code snippet. Note that we turn off the graphs for the posterior.
```{r,cache=TRUE,eval=TRUE}
coeffs = mcleod::results.covariate.coefficients.posterior(res,plot.posterior = F)
print(coeffs)
```
In addition, we plot the estimated CDF for the distribution of the $\gamma_i$'s
```{r,cache=TRUE,eval=TRUE}
plot.posterior(res)
```
...
The next example shows how run poisson regression with a general random interept.
The next code snippet generates the data, with a single covariate and a bi-modal distribution for the intercept. In addition, we include another extrinsic variable, $C_i$, so the model equation is of the form:

$$X_i\sim Pois(\lambda_i); log(\lambda_i) = \gamma_i +\beta Z_i + log(C_i).$$

The $C_i$'s are known, and we show to incorporate them into the model using an offset term. The next code snippet generates and plots the data. Note the massive differences in orders of magnitude for $X_i$'s, due to the change in $C_i$'s, which are distributed uniformly between 1 and 100.
...
```{r,cache=TRUE,eval=TRUE}
  ...
  #Plot
  hist(x,main='Dist. of Xi for the Poisson regression with offset term')
...
We obtain the posterior distribution for the slope parameter:
```{r,cache=TRUE,eval=TRUE}
...
```{r,cache=TRUE,eval=TRUE}
suppressWarnings({plot.posterior(res)})
```
